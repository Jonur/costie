...
const CurrencySwitch = () => {
  const { currencySymbol, updateCurrencySymbol } = useCostieCtx();
  ...
  return (
    <button
      aria-label={t("ariaLabel.changeCurrencySymbol")}
      type="button"
      className="absolute -top-12 rounded-full w-24 h-24 bg-primary text-white text-5xl flex flex-col items-center justify-center gap-1 cursor-pointer hover:bg-darkgrey transition-all duration-200"
      onClick={handleClick}
    >
